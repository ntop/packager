...
if [ "$1" = 'test' ]; then
    ...
    LICENSE_MGR_CONF="/usr/share/ntop/etc/license-manager-client-nprobe.conf"
    VERSION_OUTPUT=$(nprobe --license-mgr "$LICENSE_MGR_CONF" --version 2>&1)
    echo "$VERSION_OUTPUT"
    if echo "$VERSION_OUTPUT" | grep -qi "Invalid license\|License Type:.*Invalid"; then
    ...
    exec nprobe -i "$PCAP_FILE" -n none
else
